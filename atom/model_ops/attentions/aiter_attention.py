...
class AiterAttentionMetadataBuilder(CommonAttentionBuilder):
    ...
    def build(
        ...
        if num_decodes > 0 and num_prefills > 0:
            raise ValueError("ATOM does not support chunked prefill yet, please add --no-enable-chunked-prefill when launch the server")

        if num_extends > 0:
            raise ValueError("ATOM does not support split prefill yet")
