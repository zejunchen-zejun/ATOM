...
def main():
    ...
    if args.torch_profiler_dir is None:
        ...
        print("Warning: --torch-profiler-dir not specified, using default: ./profiler_traces")
    ...
    if args.random_input:
        ...
        random_token_ids = [random.randint(0, vocab_size - 1) for _ in range(args.input_length)]
        prompt = tokenizer.decode(random_token_ids, skip_special_tokens=True)
        # Re-encode to verify and truncate to exact length
        token_ids = tokenizer.encode(prompt, add_special_tokens=False)[:args.input_length]
