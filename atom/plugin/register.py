...
def register_ops_to_vllm(atom_config: Config) -> None:
    ...
    if atom_config.plugin_config.vllm_use_custom_attention:
        ...
        logger.warning("Please export VLLM_ATTENTION_BACKEND=CUSTOM to use atom attention")
...
def init_aiter_dist(config: Config) -> None:
    ...
    '''
    logger.info('Initialize aiter dist for using aiter custom collective op for plugin mode')
    ...
    logger.info(f'Initialize aiter dist for using aiter custom collective op for plugin mode, rank:{rank}')
    init_dist_env(
